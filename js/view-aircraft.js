// view-aircraft.js — CATC Aircraft Status  (Fleet · OPS Cross-Check · Utilization)
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
        ...
        <div style={{ flexShrink: 0, padding: '10px 16px', background: 'var(--bg-2)', borderBottom: '1px solid var(--line)', display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
          ...
          <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 8 }}>
            {loading && tab !== 'utilization' && <span className="mono" style={{ fontSize: 10, color: 'var(--col-pending)' }}>⟳ refreshing…</span>}
            {error   && tab !== 'utilization' && <span className="mono" style={{ fontSize: 10, color: 'var(--col-cancel)' }}>⚠ {error}</span>}
            {tab !== 'utilization' && <button className="chip" onClick={load} disabled={loading}>⟳ Refresh</button>}
          </div>
        </div>

        {/* ── Fleet summary strip (fleet/crosscheck only) ─────────────────── */}
        {tab !== 'utilization' && data && (
