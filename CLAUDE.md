...
```
**Last known:** all files `p109` (2026-07-01 — fixed Gantt NOW-line to use true Asia/Bangkok time via `Intl.DateTimeFormat` regardless of viewer's device timezone; `bkkNowMin()` moved from `view-gantt.js` into `shared.js`, same fix class as `p95`/I1's `bkkToday()`). p108 (2026-06-26 — Fleet Load Distribution now hides zero-hour tails when filtered, matching heatmap roster; `visEntries` filter). p107 (2026-06-26 — Effective metric mode for Utilization/FI Stat/SP Stat). p106 (shared.js strips "(Unplanned)" project-wide). p105 (FI Stat + SP Stat sub-tabs). p104 (Utilization: AP127 toggle + zero-row hiding). Next → `p110`. **Watchdog (2026-06-23):** `telegram.test.js` — added missing SP `@username` assertion to `STATUS → Canceled` test; all 6 notification types now verified. Implementation was already correct; test coverage gap only (no deploy needed).
