...
1. Bump `?v=pNN` token on ALL `<script>` tags in `index.html` — next must be `p206` (all currently at p205; `ops/index.html` is on its own `?v=rNN`, now r42)
...
**Last known:** all files `p205` (2026-09-19 — **Leave calendar round 2 (p205).** Removed the *Most leave* and *Booked while on leave* KPI cards (6 remain). Month name now shown above every date column in Week and Roster headers (and beside day 1 in the Month grid). Every SP/FI name (Day cards, Week rows, Roster rows, records table, record drawer) is now a link opening a **Leave history** drawer: identity line (full name, callsign, role, batch, FI), 'on leave today' banner, Records / Leave days / Taken / Upcoming stats, a reason-split bar, flight-clash count, and every record newest-first as a full card (click → the single-record drawer; closing it returns to the history). Zero-leave AP127 SPs open a 'No leave records' state.) p204 (2026-09-19 — **Schedule → new "Leave" layout: a leave calendar (p204).** New `js/view-leave.js` (plain script, loaded before `view-schedule.js`), wired as a 6th LAYOUT chip; the flight-only TYPE row hides while it is active. Built on `FLIGHT_DATA.leaves`, not on flight dates, so any date with a leave record shows (feed spans 24 Apr → 28 Oct 2026, flights end 21 Sep). Four sub-views sharing one date cursor: **Day** (a card per person with reason, full/half-day AM/PM, range + day N of M, note, the SP's FI, flights still booked), **Week** (a row per person, bars across Mon–Sun), **Month** (calendar with a headcount per day; each day opens Day view), **Roster** (people × days, half-days half-filled, all 28 AP127 SPs listed even with zero leave, sortable by most leave, a daily headcount footer). WHO filter defaults to **AP127 SP only**, with toggles for Other SP (per-batch chips) and FI (all / AP127 FIs only), plus reason-category chips (which also serve as the legend) and a search box. 8 KPI cards scoped to the visible period: on leave today, leave days, people on leave (x/28 when AP127 only), top reason, peak day, most leave, flights booked while on leave, and leave starting in the next 14 days. A record drawer shows every field incl. source ids, clashing flights and that person's full leave history. Keyboard: ← → T D W M R. **Data hygiene, in the view only (shared.js untouched):** the feed carries each leave twice (a `|rowN` sheet copy + a `|key|<ts>` form copy), so 1,041 rows → 636 records de-duped on content; names normalised to `FIRST L.` (`Watcharaphol Vongnoi`, `APIRATKUNPAK K`); 3 records with end < start swapped and flagged; leave days counted per person per day (max 1 / 0.5) so overlapping records never double-count. Verified live: all 4 views + drawer + empty-day jump, cockpit + light themes, 375px with zero horizontal overflow; `git diff --stat` empty on the DB_Share-proxied files.) p203 (2026-09-06 — **NGT_CACHE + ASF cache repointed to the
