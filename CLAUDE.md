...
1. Bump `?v=pNN` token on ALL `<script>` tags in `index.html` — next must be `p205` (all currently at p204; `ops/index.html` is on its own `?v=rNN`, now r42)
