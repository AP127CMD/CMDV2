...
```
**Last known:** all files `p107` (2026-06-26 — Effective metric mode added to Utilization, FI Stat, SP Stat tabs; `uBuildCurMap`+`uEffectiveMins` helpers wire curriculum planned_mins; filter bar Effective button, KPI label, chart Y-axis, ⚠ note). p106 (2026-06-26 — shared.js strips " (Unplanned)" project-wide). p105 (2026-06-26 — FI Stat + SP Stat: 2 new Aircraft Status sub-tabs). p104 (2026-06-26 — Utilization: AP127 toggle + hidden zero-rows). Next → `p108`. **Watchdog (2026-06-23):** `telegram.test.js` — added missing SP `@username` assertion to `STATUS → Canceled` test; all 6 notification types now verified. Implementation was already correct; test coverage gap only (no deploy needed).
