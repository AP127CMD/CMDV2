...
```
**Last known:** all files `p104` (2026-06-26 — Utilization tab: AP127-only toggle button in filter bar; heatmap now hides rows/type groups where all aircraft have 0 matching flights). p103 (2026-06-26 — Aircraft Utilization tab: 3rd tab in view-aircraft.js with heatmap roster, daily stacked chart, fleet load bars, KPI strip, full filter bar, detail drawer; block/airborne toggle, sim toggle, collapsible type groups). Next → `p105`. **Watchdog (2026-06-23):** `telegram.test.js` — added missing SP `@username` assertion to `STATUS → Canceled` test; all 6 notification types now verified. Implementation was already correct; test coverage gap only (no deploy needed).
