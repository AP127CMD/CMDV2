...
```
**Last known:** all files `p108` (2026-06-26 — Fleet Load Distribution now hides zero-hour tails when filtered, matching heatmap roster; `visEntries` filter). p107 (2026-06-26 — Effective metric mode for Utilization/FI Stat/SP Stat). p106 (shared.js strips "(Unplanned)" project-wide). p105 (FI Stat + SP Stat sub-tabs). p104 (Utilization: AP127 toggle + zero-row hiding). Next → `p109`. **Watchdog (2026-06-23):** `telegram.test.js` — added missing SP `@username` assertion to `STATUS → Canceled` test; all 6 notification types now verified. Implementation was already correct; test coverage gap only (no deploy needed).
